...
def hyperparameter_tuning(X_train, y_train):
    ...
    model = xgb.XGBClassifier(random_state=5, scale_pos_weight=7.5)
    ...
    print('\nBest parameters for Model 1:', best_params)
    print('\nBest score for Model 1:', best_score)
...
def prediction_results(X_train, y_train, X_test, y_test, best_params, ensemble=False, charts=False):
    ...
    if charts:
        ...
        plt.ylabel('Expected')
        plt.tight_layout()
        plt.savefig('images/confusion_matrix_model1.png')
        plt.close()
    ...
    if charts:
        ...
        plt.grid(True)
        plt.savefig('images/ROC_model1.png')
        plt.close()
