...
def hyperparameter_tuning(X_train, y_train):
    ...
    model = RandomForestClassifier(class_weight={0:1,1:7.5}, random_state=5)
    ...
    print('\nBest parameters for Model 2:', best_params)
    print('\nBest score for Model 2:', best_score)
...
def prediction_results(X_train, y_train, X_test, y_test, best_params, ensemble=False, charts=False):
    ...
    if charts:
        ...
        plt.ylabel('Expected')
        plt.tight_layout()
        plt.savefig('images/confusion_matrix_model2.png')
        plt.close()
    ...
    if charts:
        ...
        plt.grid(True)
        plt.savefig('images/ROC_model2.png')
        plt.close()
